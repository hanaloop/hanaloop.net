...
export default async function Page({ params }: Props) {
  ...
  if (!slug || slug.length === 0) {
    return (
      <SiteShell>
        <BlogHeroSection locale="ko" />
        <BlogListSection locale="ko" />
      </SiteShell>
    );
  }

  const post = getBlogPostContent('ko', slug);
  if (!post) notFound();

  return (
    <SiteShell>
      <article className="mx-auto w-full max-w-[980px] px-4 pb-24 pt-14 lg:px-6 lg:pt-18">
        <Link href="/blog" className="text-sm font-medium text-[#616161] hover:underline">
          Back to blog
        </Link>
...
export function generateStaticParams() {
  const slugs = getBlogPostSlugs('ko');
  return [{ slug: [] as string[] }, ...slugs.map((slug) => ({ slug }))];
}
