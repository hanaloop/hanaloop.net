...
export default function Home({ locale }: Props): React.JSX.Element {
  ...
  return (
    <Layout
      title={`${siteConfig.title} | ${t("recruit.meta_title")}`}
      description={t("recruit.meta_description")}>

      <Hero background={{
          imageUrl: useBaseUrl('/images/bg-hero_workspace.jpeg'),
        }} 
        header={t("recruit.hero_header")}
        tagline={t("recruit.hero_tagline")}
        button={
      ...
      <article className="text-left">
        ...
        <br />
        <div className="mt-4 inline-block rounded border border-primary p-2 text-lg">
          <Link to="/recruit_apply">지원하기</Link>
        </div>
      </article>
      </SectionBlock>
