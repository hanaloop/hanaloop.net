...
export default function Scope3({ locale }: Props): React.JSX.Element {
  ...
  return (
    <Layout wrapperClassName='text-gray-700 dark:text-gray-100'
      title={`${siteConfig.title} | ${t("scope3.meta_title")}`}
      description={t("scope3-ai.meta_description")}>
      <Hero background={{
          imageUrl: useBaseUrl('/images/banners/pyeongchanggun-windfarm.jpeg'),
        }} 
        header={t("scope3.hero_header")}
        tagline={t("scope3.hero_tagline")}
        button={
                      ...
                      <div className='grid xl:grid-cols-4 sm:grid-cols-2 grid-cols-1 gap-4 lg:w-[70%] w-[90%]'>
                        {
                          data.management_process.map((process, idx) => (
                            <div key={`${process.title}-${idx}`} className='w-full rounded-md p-2 flex flex-col bg-white dark:bg-gray-700 px-4'>
                        ...
                        <div className=' grid lg:grid-cols-3 lg:gap-10 sm:grid-cols-2 grid-cols-1 gap-4 justify-items-center sm:w-[80%] w-[90%]'>
                          {
                            data.articles.map((article, idx) => (
                                  ...
                                  <div key={`${article.title}-${idx}`} className='xs:w-full w-[90%] rounded-md p-2 flex flex-col bg-white dark:bg-gray-700 justify-between px-4'>
